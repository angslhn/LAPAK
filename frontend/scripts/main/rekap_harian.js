...
function renderLaporanTable(reports) {
  ...
  if (infoHeader) {
    if (openCount > 0) {
      infoHeader.innerHTML = `🔴 <strong>${openCount} hari belum ditutup</strong> dari ${reports.length} laporan`;
    } else {
      infoHeader.textContent = `🟢 Semua ${reports.length} laporan sudah ditutup`;
    }
  }
  ...
  if (btnCloseAll) {
    btnCloseAll.style.display = openCount > 0 ? 'flex' : 'none';
  }
  ...
  tbody.innerHTML = reports
    .map((r) => {
      ...
      return `
      <tr>
        <td><span class="laporan-tgl">${fmtDateShort(r.date)}</span></td>
        <td class="right"><span class="laporan-nominal">${rupiahFormatter(r.total_revenue || 0)}</span></td>
        <td class="right"><span class="laporan-nominal">${rupiahFormatter(r.total_expense || 0)}</span></td>
        <td class="right"><span class="laporan-nominal">${rupiahFormatter(r.net_profit || 0)}</span></td>
        <td class="center">${statusBadge}</td>
        <td class="center">
          <button class="aksi-link" onclick="openDetailLaporan(${r.id})">Detail</button>
          ${closeBtn}
        </td>
      </tr>`;
    })
...
async function tutupBukuPerTanggal(date) {
  if (!confirm(`Tutup buku untuk tanggal ${fmtDateShort(date)}?`)) return;
  ...
  try {
    ...
    if (data.status === 'closed' && data.closed_at) {
      ...
      document.getElementById('detailClosedBy').textContent =
        `Ditutup pada ${fmtDateLong(data.closed_at)}`;
    } else {
...
document
  ...
  .addEventListener('click', async () => {
    if (!confirm('Tutup semua laporan yang masih terbuka?')) return;
